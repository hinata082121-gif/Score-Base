      ...
      <div className="space-y-5">
        ...
        <section id="teams-output" className="overflow-x-auto rounded-md border border-stone-200 bg-white p-4 shadow-sm">
          ...
          <table className="mt-3 w-max min-w-full text-sm">
            <thead><tr className="bg-stone-100"><th className="p-2 text-left">チーム</th><th>試合数</th><th>勝</th><th>敗</th><th>分</th><th>勝率</th><th>得点</th><th>失点</th><th>本塁打</th><th>チーム打率</th><th>チームOPS</th><th>チーム防御率</th></tr></thead>
            <tbody>{rows.map((row) => <tr key={row.team} className="border-t border-stone-100"><th className="p-2 text-left">{row.team}</th><td>{row.games}</td><td>{row.wins}</td><td>{row.losses}</td><td>{row.draws}</td><td>{row.winRate}</td><td>{row.runs}</td><td>{row.allowed}</td><td>{row.hr}</td><td>{row.avg}</td><td>{row.ops}</td><td>{row.era}</td></tr>)}</tbody>
          </table>
