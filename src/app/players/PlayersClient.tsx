          ...
          {filtered.map((player) => (
            <article key={player.id} className="rounded-md border border-stone-200 bg-white p-4 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="text-sm font-bold text-emerald-700">{player.teamName || "未所属"} / #{player.number || "-"}</p>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <button className="rounded-md bg-red-50 px-3 py-2 text-sm font-bold text-red-700" onClick={() => remove(player.id)}>削除</button>
                </div>
              </div>
            </article>
          ))}
