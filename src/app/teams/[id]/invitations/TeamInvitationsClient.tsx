...
export function TeamInvitationsClient({ id, teamName, currentRole, invitations }: { id: string; teamName?: string; currentRole?: TeamRole | null; invitations: InvitationRow[] }) {
  ...
  const [form, setForm] = useState({ email: "", role: "VIEWER" as TeamRole, expiresInDays: "7" });
  ...
  function invite(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await createInvitationAction(id, form.role, form.email || undefined, form.expiresInDays);
      if (!result.ok) {
      ...
      setForm({ email: "", role: "VIEWER", expiresInDays: "7" });
  ...
  return (
    <PageShell title={`${teamName} 招待`} lead="DB保存の招待リンクを発行し、受諾状態を記録します。">
      <div className="space-y-4">
        <form onSubmit={invite} className="grid gap-3 rounded-md border border-stone-200 bg-white p-4 shadow-sm sm:grid-cols-[1fr_140px_140px_auto] sm:items-end">
          {message ? <p className="rounded-md bg-emerald-50 p-3 text-sm font-bold text-emerald-800 sm:col-span-4">{message}</p> : null}
          ...
          <label className="text-sm font-bold text-stone-700">期限<select className={field} value={form.expiresInDays} onChange={(event) => setForm({ ...form, expiresInDays: event.target.value })}>{expiryOptions.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label>
            ...
            return (
              <div key={invitation.id} className="rounded-md border border-stone-200 bg-white p-4 shadow-sm">
                <div className="flex flex-wrap items-center gap-2">
                  <p className="mr-auto text-sm font-bold text-stone-700">{invitation.email || "メール未指定"} / {invitation.role} / {expired ? "EXPIRED" : invitation.status}</p>
                ...
                <p className="mt-2 text-xs font-bold text-stone-500">作成 {invitation.createdAt.slice(0, 10)} / 期限 {invitation.expiresAt ? invitation.expiresAt.slice(0, 10) : "無期限"} / 受諾 {invitation.acceptedAt ? invitation.acceptedAt.slice(0, 10) : "-"}</p>
              </div>
            );
          })}
