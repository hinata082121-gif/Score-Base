...
import { registerUser } from "@/lib/auth/clientAuth";
...
export function RegisterClient() {
  ...
  return (
    <PageShell title="新規登録" lead="チーム共有や招待を使うためのScore Baseアカウントを作成します。">
      <form onSubmit={submit} className="mx-auto grid max-w-md gap-4 rounded-md border border-stone-200 bg-white p-4 shadow-sm">
        ...
        <label className="text-sm font-bold text-stone-700">表示名<input className={field} required value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label>
        ...
        <label className="text-sm font-bold text-stone-700">パスワード確認<input className={field} type="password" required value={form.confirm} onChange={(event) => setForm({ ...form, confirm: event.target.value })} /></label>
        <button className="min-h-12 rounded-md bg-emerald-700 px-4 text-sm font-bold text-white disabled:opacity-60" type="submit" disabled={saving}>{saving ? "登録中" : "登録する"}</button>
        <p className="text-center text-sm text-stone-600">登録済みの場合は <Link className="font-bold text-emerald-700" href="/login">ログイン</Link></p>
      </form>
    </PageShell>
  );
}
