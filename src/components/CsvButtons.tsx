...
export function CsvDownloadButton({ filename, getCsv, label = "CSV保存", onDownload }: { filename: string; getCsv: () => string; label?: string; onDownload?: () => void | Promise<void> }) {
  return (
    <button
      type="button"
      onClick={async () => {
        downloadText(filename, getCsv());
        await onDownload?.();
      }}
