...
現在のmigration filesはローカルSQLite開発から作成されています。PostgreSQLをProduction DBとして使う場合は、初回本番投入前にprovider切替とmigration方針を確認し、Productionの`DATABASE_URL`接続先を必ず確認してから `migrate deploy` を実行してください。
