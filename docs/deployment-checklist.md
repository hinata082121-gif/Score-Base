...
- Verify `/games/new/watch`, `/games/new/simple`, `/games/new/scorebook`, `/games/[id]/edit`, and `/games/[id]/scorebook`.
- Confirm every mode starts with the game info registration step and can save game information before final result entry.
- Confirm `/games` shows input status and a resume action for partially entered games.
- Confirm detailed scorebook mode proceeds through home lineup, visitor lineup, lineup confirmation, game start, live input, and result entry.
- Check iPhone SE width for no page-level horizontal overflow on lineup and result steps.
- Confirm result entry records final score, outcome, game status, reason memo, standout player, MVP, and memo.
- Confirm RunnerEvent remains a known next-phase item and runner state is still checked through PlateAppearance before/after fields.

## v0.7.13 Notes

- Confirm all new/edit game routes no longer log Maximum update depth errors after deploy.
- Confirm required/optional badges and validation summary display on game info and result entry.
- Confirm weather has fixed options only and no create option.
- Confirm venue and competition candidates reuse saved Game values after creating a new record.
- Confirm score `0` can be cleared, empty blur returns to `0`, and typed digits do not reset to `0`.
- Confirm simple mode score-only and scoreboard modes both save and reload.
- Confirm detailed lineup candidates never include players from the opposite selected team.
- Confirm bottom action bars do not overlap the mobile bottom nav.
- Confirm `/games/[id]/edit` shows DB retry UI for DB load failures instead of silently falling back to localStorage.

## v0.7.14 Notes

- Local `HEAD` and `origin/main` are both `85466ce6c8cd51b90c691f07176c6113de248337`.
- Confirm the Vercel Production deployment manually in Dashboard: Status `Ready`, Environment `Production`, Branch `main`, Production domain `score-base.vercel.app`, and Source commit `85466ce` or later.
- This workspace could not verify the exact Vercel deployment commit because `.vercel/project.json` is absent and the Vercel CLI token is invalid.
- Do not infer the Production commit from public HTML.
- Public route sweep passed for `/`, `/games`, `/games/new/watch`, `/games/new/simple`, `/games/new/scorebook`, and `/login` with no captured browser console error/warn.
- 320px-class checks for `/games` and `/games/new/scorebook` showed no page-level horizontal overflow.
- Run the authenticated DB-backed smoke manually after confirming the deployment: watch-only save/edit, simple score-only, simple scoreboard, detailed SCOREBOOK through result, reload, logout/relogin, and User B role sweep.
- Inspect Vercel Runtime Logs for Error/Fatal entries on `/games/new/watch`, `/games/new/simple`, `/games/new/scorebook`, `/games/[id]/edit`, `/games`, `/games/[id]`, `/games/[id]/scorebook`, and `/games/[id]/export`.
