...
- `/games` の入力状況ラベルを確認します。試合情報だけ保存した記録は「スタメン入力待ち」または「試合結果待ち」として表示されます。
- 詳細スコアブックは `/games/[id]/scorebook`、観戦記録のみ/簡易記録は `/games/[id]/edit` から再開します。
- DB保存済みGameが表示されない場合は、ログイン状態、TeamMember role、`/settings/deployment` のPrisma接続診断を確認します。
- 未ログインのゲスト記録はlocalStorageに保存されるため、同じブラウザ/端末で確認します。

## 試合結果入力に進めない

- 観戦記録のみ/簡易記録では、試合情報登録後に「保存して試合結果へ」を使います。
- 詳細スコアブックでは、スタメン確認、試合開始、スコアブック入力を経由してから上部の「試合結果へ」を使います。
- 試合開始前に結果を入れたい場合でも、詳細スコアブックではスタメン最終確認を先に通す運用にします。

## 保存ボタンが押せない、または保存中のまま戻らない

- v0.7.13以前は、`GameForm` のデフォルト配列がレンダーごとに新規作成され、候補読み込みeffectが再実行ループになる場合がありました。
- 保存状態が「保存中」のまま戻らない場合は、Consoleに `Maximum update depth exceeded` が出ていないか確認します。
- 必須項目不足の場合は保存ボタンを常時disabledにせず、クリック後にvalidation summaryと項目別エラーを表示します。
- 下部操作バーがモバイル下部ナビに重なっていると、保存ボタンではなくナビをタップすることがあります。v0.7.13以降は操作バーを下部ナビの上へ配置します。

## スコアの0が消せない

- v0.7.13以降、最終スコアは文字列draftで保持します。
- 入力中は空文字を許可し、blur時に空欄なら0へ戻します。
- 保存時は空文字を0として扱います。
- スコアボード方式や詳細記録では合計点はイニング別入力から計算します。

## DB保存済み編集がlocalStorage扱いになる

- v0.7.13以降、DB IDの取得失敗はDBエラーとして扱い、localStorage fallbackへ落としません。
...
- v0.7.8確認ではPC幅、iPhone SE相当、iPhone 11相当でページ全体の横overflowは出ていません。再発時はスコアボード外の固定幅要素も確認してください。

## 登録済み選手がスタメン候補に出ない

- 試合詳細入力で登録済みTeamを選択しているか確認します。
- `Player.teamId` が選択したTeamのIDと一致しているか確認します。
- チーム詳細から作成したPlayerは、完了後に `/players` と `/teams/[teamId]` が再検証されます。
- v0.7.8確認では、Productionで作成した `SB_TEST_PLAYER_20260612_V078` が詳細スコアブックのスタメン入力候補に表示されました。
